        ...
        switch (expected_type){
          ...
          case "enums":
            return _.chain(field_templates)
              ...
              .thru(
                (enum_values) => {
                  const some_submitted_values_are_not_enum_options = _.without(submitted_value, ...enum_values).length > 0;
                  const required_field_but_no_valid_values = is_required && _.intersection(submitted_value, enum_values).length === 0;
                  const radio_form_but_multiple_values = field_templates[field_key].form_type === "radio" && submitted_value.length > 1;
