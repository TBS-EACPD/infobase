...
if [[ ! -z "$inactive_branches_with_live_dev_links" ]]; then
  ...
  while IFS= read -r branch_name; do
    gcloud storage rsync --continue-on-error --recursive --delete-unmatched-destination-objects --predefined-acl=publicRead --project=$DEV_CLIENT_PROJECT_ID "$dead_dev_link_html_location" "$GCLOUD_BUCKET_ROOT/$branch_name"
