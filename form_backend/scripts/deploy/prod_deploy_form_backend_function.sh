...
gcloud beta functions deploy prod-email-backend --max-instances 1 --project ${PROJECT} --region us-central1 --entry-point form_backend --stage-bucket email-backend-staging-bucket --runtime nodejs20 --trigger-http --allow-unauthenticated --env-vars-file $scratch/envs.yaml > /dev/null
