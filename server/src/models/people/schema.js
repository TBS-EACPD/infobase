...
export default function({models}){
  ...
  const headcount_models_by_name_in_schema = _.chain(snake_case_headcount_model_names)
    .zip(camel_case_headcount_model_names)
    .fromPairs()
    .mapValues( model_name => models[model_name] )
    .value();

  function get_flat_headcount_by_dimension_data_all_dimensions(subject, model_name){
    
    let records;
    
    if(subject.level === "org"){
      records = headcount_models_by_name_in_schema[model_name].get_dept_records(subject.dept_code);
    } else if(subject.level === 'gov'){
      records = headcount_models_by_name_in_schema[model_name].get_gov_records();
    }
  
    return _.chain(records)
      .flatMap( record => _.map(people_years, 
        year => ({
          year,
  ...
  function get_flat_headcount_by_dimension_data_one_dimension(subject, model_name, dimension_value){
    ...
    records = _.filter(records, record => record.dimension === dimension_value);
  ...
  function get_flat_headcount_by_dimension_data(subject, model_name, dimension_value) {
    const flat_headcount_data = dimension_value === "all_dimensions" ?
      get_flat_headcount_by_dimension_data_all_dimensions(subject, model_name) :
      get_flat_headcount_by_dimension_data_one_dimension(subject, model_name, dimension_value);  
  ...
  function headcount_by_dimension_avg_share_resolver({flat_headcount_data, subject, model_name, dimension_value}){
    ...
    if (dimension_value === "all_dimensions") {
      ...
      const all_years_total = _.reduce(flat_headcount_data, (total, d) => total + d.headcount, 0);
      ...
      const { avg_share } = headcount_by_dimension_basic_trend_resolver({flat_headcount_data, subject, model_name, dimension_value});
  ...
  function headcount_by_dimension_basic_trend_resolver({flat_headcount_data, subject, model_name, dimension_value}){
  
    const flat_headcount_all_dimensions_data = dimension_value === "all_dimensions" ? 
      flat_headcount_data :
      get_flat_headcount_by_dimension_data_all_dimensions(subject, model_name);
  
    const flat_headcount_totals_data =_.chain(people_years)
      .map( year => {
        const year_total = _.reduce(flat_headcount_all_dimensions_data, (total, record) => {
    ...
    const basic_headcount_totals_trend = get_headcount_totals_basic_trend({subject, flat_headcount_totals_data});
    ...
    const totals = _.chain(flat_headcount_data)
      ...
      .map( (group, year) => [
        year,
        _.sumBy(group, 'headcount'),
      ])
      .fromPairs()
      .value();
    
    const avg_headcount = _.chain(people_years)
    ...
    const change_percent = (totals[last_active_year] - totals[first_active_year])/totals[first_active_year];
  
    return _.assign(totals, 
      {
  ...
  function get_headcount_totals_basic_trend({subject, flat_headcount_totals_data}){
    ...
    const years_with_headcounts = _.filter(flat_headcount_totals_data, d => d.headcount !== 0);
    ...
    if (years_with_headcounts.length === 0){
      ...
      const active_year_count = _.indexOf(people_years, last_active_year) - _.indexOf(people_years, first_active_year) + 1;
      
      const total_avg_headcount = _.reduce(overall_totals, (full_total, year_total) => full_total + year_total)/active_year_count;
      
      const total_change_percent = (overall_totals["total_"+last_active_year] - overall_totals["total_"+first_active_year])/overall_totals["total_"+first_active_year];
  ...
  function employee_headcount_subject_data_resolver(model_name, subject, {dimension}){
    const flat_headcount_data = get_flat_headcount_by_dimension_data(subject, model_name, dimension);
    return { flat_headcount_data, subject, model_name, dimension_value: dimension };
  };
  const headcount_model_subject_data_resolvers = _.chain(headcount_models_by_name_in_schema)
    .keys()
    .map( headcount_model_name => {
      const model_subject_data_resolver_name = headcount_model_name + "_data";
      const model_subject_data_resolver = _.curry(employee_headcount_subject_data_resolver)(headcount_model_name);
  ...
  function employee_type_subject_info_resolver(subject){
    const flat_headcount_data = get_flat_headcount_by_dimension_data(subject, "employee_type", "all_dimensions");
    ...
    const {
      ...
      flat_headcount_all_dimensions_data,
    } = headcount_by_dimension_basic_trend_resolver({ flat_headcount_data, subject, model_name: "employee_type", dimension_value: "all_dimensions" });
  ...
  const subject_resolvers = _.extend({}, headcount_model_subject_data_resolvers, subject_info_resolvers);
