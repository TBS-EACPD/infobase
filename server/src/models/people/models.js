...
export default function define_resource_models(model_singleton){
  ...
  class HeadcountModel {
    ...
    static register(obj){
      ...
      _headcount_records_by_model_and_dept_code[model_name][dept_code].push(inst);

    }
    static get_dept_records(dept_code){
      const model_name = this.get_model_name();
      return _headcount_records_by_model_and_dept_code[model_name][dept_code] || [];
    }
    static get_gov_records(){
      const model_name = this.get_model_name();
      return _headcount_records_by_model_and_dept_code[model_name]["ZGOC"] || [];
    }
  }
  ...
  camel_case_headcount_model_names.forEach( model_name => headcount_model_factory(model_name) );
}