...
export default function({models}){
  ...
  const estimates_records = get_standard_csv_file_rows("estimates_votestat.csv");

  const program_vs_records = get_standard_csv_file_rows("program_vote_stat.csv");
  ...
  _.each(estimates_records, record => {
    _.each(estimates_years, col => record[col] = record[col] && parseFloat(record[col]));
  });
