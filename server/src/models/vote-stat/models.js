...
function flatten_pa_records(records){
  return _.chain(records)
    .flatMap(record => _.map(public_account_years, year => ({
      year,
      auth: record[`${year}_auth`],
      exp: record[`${year}_exp`],
      ..._.pick(record, ['dept_code', 'vote_num', 'vs_type', 'name_en', 'name_fr']),
    })))
    .value();
}
...
function flatten_estimates_records(records){
  return _.chain(records)
    .flatMap(record => _.map(estimates_years, year => ({
      year,
      amount: record[year],
      ..._.pick(record, ['dept_code', 'vote_num', 'vs_type', 'name_en', 'name_fr', 'doc']),
    })))
    .value();
}
...
function flatten_program_records(records, years_to_include=program_years){
  ...
  return _.chain(records)
    .flatMap( row => _.map(years_to_include, year => ({
      year,
      vs_type: row.vs_type,
      exp: row[year],
    })
...
export default function define_resource_models(model_singleton){
  ...
  class ProgramVoteStat {
    ...
    static get_flat_program_records(program_id,years){
      return flatten_program_records(_vote_stat_records_by_program_id[program_id], years);
    }
  }
