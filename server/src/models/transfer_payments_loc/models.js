...
const loc_inside_map_view = (coords, circle) => {
  ...
  return circle.lat >= coordsSW.lat  &&  circle.lat <= coordsNE.lat  &&  isLongInRange;
}
...
export default function(model_singleton){
  ...
  class TransferPaymentLocationRecord {
    ...
    static get_total_amount(location_code) {
      return  parseFloat(_.sumBy(_records_by_loc_code_transfer_payment[location_code], 'amount'));
    }
    static get_records_by_laglng(coords) {
      const filtered_records = all_loc_def_records.filter(rec => loc_inside_map_view(coords, {lat: rec.lat, lng: rec.lng}));
      const codes_in_map_view = _.uniqBy(filtered_records, 'location_code').map(rec => rec.location_code);
      const filtered_transfer_payment_records = all_transfer_payment_records.filter(rec => codes_in_map_view.includes(rec.location_code));
  ...
  model_singleton.define("TransferPaymentLocationRecord", TransferPaymentLocationRecord);
