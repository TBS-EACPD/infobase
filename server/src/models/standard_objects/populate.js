...
export default function({models}){
  ...
  const prog_records =  csv_prog_records.map(obj => _.assign({}, obj, {
    program_id: create_program_id(obj),
    so_num: +obj.so_num,
  }));

  const org_records = csv_org_records.map(obj => _.assign({}, obj, { 
    so_num: +obj.so_num,
  }));


  _.each(org_records, record => {
    _.each(public_account_years, col => record[col] = record[col] && parseFloat(record[col]));
  })

  const prog_sobj_cols = _.takeRight(public_account_years, 2);
  _.each(prog_records, record => {
    _.each(prog_sobj_cols, col => record[col] = record[col] && parseFloat(record[col]));
  })
