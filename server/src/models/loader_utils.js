...
const deep_and_complete_group_by = (attr_path, ids, rows) => {
  ...
  const get_rows_that_matched_on_id = (id) => _.filter(
    rows,
    row => _.chain(keys_in_attr_path)
      .reduce(
        (documents, attr) => {
  ...
  const groups_of_all_matches_by_id = _.chain(ids)
    .map(
      id => [
        id,
        get_rows_that_matched_on_id(id),
      ]
    )
    .fromPairs()
    .mapValues(matched_rows => _.isEmpty(matched_rows) ? null : matched_rows)
    .value();
