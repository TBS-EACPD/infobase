...
gcloud functions deploy $CIRCLE_BRANCH --entry-point app --stage-bucket $CIRCLE_BRANCH-staging-bucket --runtime nodejs6 --trigger-http --env-vars-file ./envs.yaml > /dev/null