if [[ $CIRCLECI && $REDACT_LOGS && ($1 == "redact-start") ]] ; then
  ...
  function redact_env_vars_from_file(){
    ...
    env | grep -v "^[$env_whitelist_pattern]=" > $filtered_env_file
    ...
    # sort the maping file by the length of the env var value, don't want to only redact a sub-string of a longer env var by coincidence
