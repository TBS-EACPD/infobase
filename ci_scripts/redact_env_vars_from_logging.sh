if [[ $CIRCLECI && $REDACT_LOGS && ($1 == "redact-start") ]] ; then
  ...
  function redact_env_vars_from_file(){
    ...
    env_whitelist_pattern="(CI|CIRCLECI|CIRCLE_BRANCH|CIRCLE_BUILD_NUM|CIRCLE_BUILD_URL|CIRCLE_COMPARE_URL|CIRCLE_JOB|CIRCLE_NODE_INDEX|CIRCLE_NODE_TOTAL|CIRCLE_PREVIOUS_BUILD_NUM|CIRCLE_PROJECT_REPONAME|CIRCLE_PROJECT_USERNAME|CIRCLE_REPOSITORY_URL|CIRCLE_SHA1|CIRCLE_SHELL_ENV|CIRCLE_STAGE|CIRCLE_USERNAME|CIRCLE_WORKFLOW_ID|CIRCLE_WORKFLOW_JOB_ID|CIRCLE_WORKFLOW_UPSTREAM_JOB_IDS|CIRCLE_WORKFLOW_WORKSPACE_ID|CIRCLE_WORKING_DIRECTORY|SHLVL|REDACT_LOGS|SHOULD_USE_REMOTE_DB)"
