...
const split_matched_search_tokens = (search, content) => {
  ...
  _.each(
    reg_exps,
    (reg_exp) => modified_string = modified_string.replace(reg_exp, match => `${split_token}${match}${split_token}`)
  );
...
class InfoBaseHighlighter extends React.Component {
  render(){
    ...
    const string_split_on_matched_tokens = split_matched_search_tokens(search, content);
