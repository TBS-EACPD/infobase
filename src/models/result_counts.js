...
function compute_counts_from_set({results, indicators}){
  const drr17_indicators = _.filter(indicators, { doc: 'drr17'}),
    drr17_indicators_past_met = _.filter(drr17_indicators, {status_key: 'met'}).length,
    drr17_indicators_past_not_met = _.filter(drr17_indicators, {status_key: 'not_met'}).length,
    drr17_indicators_past_not_reported = _.filter(drr17_indicators, {status_key: 'not_reported'}).length,

    drr17_indicators_future_to_be_achieved = _.filter(drr17_indicators, {status_key: 'to_be_achieved'}).length,
