...
const prg_crso_split_render = function({calculations, footnotes}){
  ...
  const { programSpending_data } = graph_args;
  ...
  const unfiltered_data = _.chain(programSpending_data)
    .map(d => ({ 
      value: d[col],
      label: d.prgm,
      data: d[col],
...
new PanelGraph({
  ...
  info_deps: ['programSpending_crso_info'],
  ...
  calculate(subject,info){
    ...
    const programSpending_data = programSpending.q(subject).data;

    if(d3.sum(_.map(programSpending_data, yr => yr["{{planning_year_1}}"])) === 0){
      return false
    } 

    return { programSpending_data };
