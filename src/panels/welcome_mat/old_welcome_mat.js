...
new PanelGraph({
  level: "gov",
  key: 'old_welcome_mat',
  depends_on: ['orgVoteStatPa','programSpending','programFtes'],
  info_deps: [ 'orgVoteStatPa_gov_info', 'programSpending_gov_info','programFtes_gov_info'],
  layout: {
  ...
  calculate(subject,info,options) {
    ...
    return calculate({
      ...
      exp: {
        ...
        graph_data: programSpending.q().sum(c.exp , {as_object: false}),
      },
      fte: {
        was: info.gov_fte_pa_last_year_5,    
        changed_to: info.gov_fte_pa_last_year,      
        will_be: info.gov_fte_planning_year_3,
        graph_data: programFtes.q().sum(c.fte, {as_object: false}),
      },
      footers: {
...
new PanelGraph({
  ...
  depends_on: [ 'programSpending','programFtes', 'orgVoteStatPa', 'orgVoteStatEstimates'],
  info_deps: ['orgVoteStatPa_dept_info', 'programSpending_dept_info','programFtes_dept_info', 'orgVoteStatEstimates_dept_info'],
  missing_info: 'ok' ,
  layout: {
    full: {text: 12, graph: 0},
    half: {text: 12, graph: 0},
  ...
  calculate(subject,info,options) {
    ...
    return calculate({
      ...
      exp: {
        ...
        graph_data: programSpending.q(subject).sum(c.exp , {as_object: false}),
      },
      fte: {
        ...
        graph_data: programFtes.q(subject).sum(c.fte, {as_object: false}),
      },
      footers: {
...
new PanelGraph({
  ...
  info_deps: [ 'programSpending_program_info','programFtes_program_info'],
  missing_info: 'ok' ,
  layout: {
    full: {text: 12, graph: 0},
    half: {text: 12, graph: 0},
  ...
  calculate (subject,info,options) {
    ...
    if (programSpending.programs.get(subject) === undefined ){

      return false;
    }
    const pd = subject.has_planned_spending && programFtes.programs.get(subject) !== undefined ;

    let exp_graph_data = _.map(c.exp,col => _.first(programSpending.programs.get(subject))[col]); 
    let fte_graph_data = pd ? _.map(c.fte,col => _.first(programFtes.programs.get(subject))[col]) : undefined;
    const new_program = (
...
new PanelGraph({
  ...
  footnotes: ["MACHINERY"],
  depends_on: [ 'programSpending','programFtes'],
  info_deps: [ 'programSpending_tag_info','programFtes_tag_info'],
  missing_info: 'ok' ,
  layout: {
    full: {text: 12, graph: 0},
    half: {text: 12, graph: 0},
...
new PanelGraph({
  ...
  calculate (subject,info,options) {
    ...
    if (this.tables.programSpending.q(subject) === undefined ){
      return false;
    }

    const crso_exp_data = this.tables.programSpending.q(subject).data;
    const crso_fte_data = this.tables.programFtes.q(subject).data;
    ...
    // let exp_graph_data =  _.map(c.exp, col => this.tables.programSpending.q(subject)[col]); 
    let exp_graph_data = _.map(c.exp, function(row) {
      return d3.sum(_.map(crso_exp_data, xx => xx[row]))
    });

    // let fte_graph_data =  pd ? _.map(c.fte,col => this.tables.programFtes.q(subject)[col]) : undefined;
