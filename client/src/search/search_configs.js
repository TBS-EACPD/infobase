...
const org_templates = {
  ...
  name_function: org => org.applied_title ? `${org.applied_title} (${_.toLower( trivial_text_maker('legal_name') )}: ${org.name})` : org.display_name,
  menu_content_function: function(org, search){
    if (org.level !== "gov" && org.old_name){
      ...
      const menu_content_with_old_name = `${org.display_name} (${trivial_text_maker("previously_named")}: ${org.old_name})`;
...
const gocos = {
  ...
  name_function: _.property('display_name'),
...
const program_or_crso_search_display_name = ({is_internal_service, display_name, dept}) => is_internal_service ?
  display_name :
