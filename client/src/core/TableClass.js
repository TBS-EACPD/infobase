...
export class Table extends mix().with(staticStoreMixin){
  ...
  get links(){
    return ( 
      ...
      _.map(this.source, source_key => all_sources[source_key].open_data[window.lang] )
    );
  }
  get description(){
  ...
  header_structure(col_nicks){
    ...
    const ancestor_cols = col => col.parent ? [ ...ancestor_cols(col.parent),col ] : [col];

    const id_for_col = col => prefix+(col.nick || col.wcag);

    //given a col, returns the header string that points TO it. 
    const header_to_col = col => id_for_col(col) + (col.parent ? ' '+header_to_col(col.parent) : '');

    const colspan_for_col = col => _.chain(col_nicks)
    ...
    const col_structure = _.chain(col_nicks)
      ...
      .groupBy('level')
      .map( group => _.sortBy(group,obj => _.indexOf(this.flat_headers, this.col_from_nick(obj.nick) ) ) )
      .value();

    const headers_for_cell = _.chain(col_nicks)
      .map( col_nick => [ col_nick, header_to_col(this.col_from_nick(col_nick)) ] )
      .zipObject()
      .value();
  ...
  col_from_nick(nick){
    ...
    return _.find(this.flat_headers, col => col.nick === nick || col.wcag === +nick ) || false;
  }
  add_col(x){
