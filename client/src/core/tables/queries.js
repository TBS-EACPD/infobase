...
class Queries {
  ...
  get_cols(cols, options){
    ...
    if (gross_percentage){
      ...
      vals[gp_colname] = [];
      var sum = _.chain(vals[gross_percentage])
    ...
    if (options.zip){
      vals = _.zip.apply(null, _.map(cols,function(col){return vals[col];}));
    }
    return vals;
  }
  ...
  get_rows(criteria, options) {
    ...
    //
    var each_mapped_obj = obj => (
      _.every(_.toPairs(criteria).map( ([key,val]) => {
...
function query_adapter(subject){
  ...
  if (subject && subject === Subject.Gov){
    subject = undefined;
  } else if (this.programs && subject && _.includes(['tag', 'crso'], subject.level )){
