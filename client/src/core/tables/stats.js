...
const stats = {
  ...
  year_over_year_multi_stats_active_years(
    add,
    context_prefix,
    amounts,
    act_years,
    ...
    if (act_years) {
      ...
      amounts_temp = _.map(amounts_temp, (amounts_temp) =>
        _.tail(amounts_temp)
      );
      ...
      var m = run_template;
      var first_active_year_index = _.findIndex(
        amounts_temp,
        (amounts_temp) => amounts_temp !== 0
      );
      var last_active_year_index = _.findLastIndex(
        amounts_temp,
        (amounts_temp) => amounts_temp !== 0
      );
      if (first_active_year_index > 0) {
        add(
          context_prefix + "_first_active_year",
          m(years[first_active_year_index])
        );
      } else {
      ...
      if (last_active_year_index > 0) {
        add(
          context_prefix + "_last_active_year",
          m(years[last_active_year_index])
        );
      } else {
        add(context_prefix + "_last_active_year", m(years[4]));
      }
      add(
        context_prefix + "_number_active_years",
        last_active_year_index - first_active_year_index + 1
      );
      amounts_temp = _.countBy(amounts_temp, function (amounts_temp) {
        return amounts_temp === 0 ? "inactive" : "active";
      });
