...
const get_single_info = _.memoize(
  (stats_key, subject) => {
    ...
    if (stats.level === "gov") {
      ...
      (subject.is("program") || subject.is("crso")) &&
      stats.level === "dept"
    ) {
...
function get_info(subject, infokeys) {
  const computed_separate = _.map(infokeys, (key) =>
    get_single_info(key, subject)
  );
...
class Statistics extends mix().with(staticStoreMixin) {
  ...
  report_on(subject) {
    ...
    const tables = _.chain(this.table_deps)
      .map((id) => [id, Table.lookup(id)])
      .fromPairs()
      .value();
    const infos = _.chain(this.info_deps)
      .map((info_id) => {
...
export { Statistics, get_info, tables_for_statistics };
