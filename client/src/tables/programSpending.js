...
const { std_years, extended_planning_years, planning_years } = year_templates;
...
export default {
  ...
  title: {
    en:
      "Expenditures and Planned Spending by Program from {{pa_last_year_5}} to {{planning_year_3}} ($)",
    fr:
      "Dépenses réelles et prévues par programme de {{pa_last_year_5}} à {{planning_year_3}} (en dollars)",
  },
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    var last_year = q.get_top_x(["prgm", "{{pa_last_year}}exp"], Infinity, {
      zip: true,
      sort_col: "{{pa_last_year}}exp",
    });
    ...
    stats.year_over_year_multi_stats(
      add,
      "prgm_exp_spend_areas",
      all_years_spend_areas
    );

    add("prg_num", last_year_prg_num);

    stats.add_all_years(add, "exp", extended_planning_years, (year, i) =>
      q.sum(year)
    );
    ...
    const crso_num = subject.crsos.length;
    ...
    const CRSO_data = _.map(crsos, (crso) => ({
      crso: crso,
      data: d3.sum(
        _.map(table.q(crso).data, (prg) => prg["{{planning_year_1}}"])
      ),
    }));
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    add(
      "dept_exp_pa_last_year",
      infos.orgVoteStatPa_dept_info.dept_exp_pa_last_year
    );
    add(
      "pct_of_dept_exp_pa_last_year",
      c.program_exp_pa_last_year /
        infos.orgVoteStatPa_dept_info.dept_exp_pa_last_year
    );
    ...
    add(
      ...
      c.program_exp_planning_year_1 /
        infos.programSpending_dept_info.dept_exp_planning_year_1
    );
    add(
      "dept_exp_planning_year_1",
      infos.programSpending_dept_info.dept_exp_planning_year_1
    );
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    var last_year = _.map(
      q.get_top_x(["dept", "activity_code", "{{pa_last_year}}exp"], Infinity, {
        zip: true,
        sort_col: "{{pa_last_year}}exp",
      }),
      ([org_id, ac, val]) => [
        `${Dept.lookup(org_id).name} - ${
          Program.get_from_activity_code(org_id, ac).name
        }`,
        val,
      ]
    );
    ...
    stats.add_all_years(add, "exp", std_years, (year, i) =>
      q.sum(year + "exp")
    );
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    var first_year_prg_num = _.filter(q.data, function (d) {
      return d[_.first(extended_planning_years)] !== 0;
    }).length;
    ...
    const min_planning_yr =
      "{{planning_year_" +
      _.min(_.map(extended_planning_years, (yr) => Number(yr.match(/\d+/)))) +
      "}}";
    ...
    const data = _.map(programSpending_data, (d) => ({
      value: d["{{planning_year_1}}"],
      label: d.prgm,
      id: d.activity_code,
      link: Program.lookup(Program.unique_id(d.dept, d.activity_code))
        .link_to_infographic,
    }));
