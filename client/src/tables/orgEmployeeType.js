      ...
      this.add_col({
        ...
        description: {
          en:
            ...
            people_years_short_second[ix],
          fr:
            "Correspond à personnel actif par type d'employé, au 31 mars " +
            people_years_short_second[ix],
        },
      });
    });
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    add(
      "head_count_dept_share_last_year",
      dept_last_year /
        _.reduce(
          table.data,
          (memo, d) => {
    ...
    stats.one_year_top3(
      add,
      "emp_types",
      q.get_top_x(["employee_type", col], {
        zip: true,
        sort_col: col,
      })
    );
    const all_years = q.get_top_x(
      ["employee_type"].concat(people_years),
      Infinity,
      { zip: true }
    );
    ...
    add(
      "head_count_avg",
      d3.sum(total_head_count_by_year) /
        _.filter(total_head_count_by_year, (d) => d > 0).length
    );

    const first_active_year_index = _.findIndex(
      total_head_count_by_year,
      (d) => d !== 0
    );
    ...
    const students = _.find(
      q.data,
      (d) => d.employee_type === "Student" || d.employee_type === "Étudiant"
    );

    add(
      "student_share_first_active_year",
      first_active_year_index !== -1 && students !== undefined
        ? students[people_years[first_active_year_index]] /
            total_head_count_by_year[first_active_year_index]
        : 0
    );
    ...
    add(
      ...
      first_active_year_index !== -1 && students !== undefined
        ? students[people_years[last_active_year_index]] /
            total_head_count_by_year[last_active_year_index]
        : 0
    );
  },
});
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    add("student_share_last_year", students[5] / yearly_total_head_counts[4]);
  },
});
