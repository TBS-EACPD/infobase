...
Statistics.create_and_register({
  id: "orgVoteStatPa_dept_info",
  table_deps: ["orgVoteStatPa"],
  level: "dept",
  compute: (subject, tables, infos, add, c) => {
    const table = tables.orgVoteStatPa;
    ...
    const unused_auth_diff = _.map(std_years, (year) =>
      _.map(q.data, (row) => row[year + "auth"] - row[year + "exp"])
    );
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    add(
      "stat_five_year_pct",
      c.gov_stat_total / (c.gov_stat_total + c.gov_voted_total)
    ); //type percentage
    add(
      "voted_five_year_pct",
      c.gov_voted_total / (c.gov_stat_total + c.gov_voted_total)
    ); //type : "percentage"
  },
});
