      ...
      this.add_col({
        ...
        description: {
          en:
            ...
            people_years_short_second[ix],
          fr:
            "Correspond à le personnel actif par direction niveaux, au 31 mars " +
            people_years_short_second[ix],
        },
      });
    });
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    const all_years = q.get_top_x(["ex_lvl", ...people_years], Infinity, {
      zip: true,
    });
    stats.year_over_year_multi_stats_active_years(
      add,
      "head_count_ex_level_years",
      all_years,
      false,
      people_years
    );
    ...
    if (!_.isEmpty(all_years_only_ex)) {
      ...
      add(
        ...
        (ex_lev_EX_avg * num_active_years) /
          d3.sum(_.map(all_years, (a) => d3.sum(a.slice(1))))
      );
    } else {
...
Statistics.create_and_register({
  ...
  compute: (subject, tables, infos, add, c) => {
    ...
    add(
      "head_count_ex_avg_share",
      d3.sum(year_totals) / d3.sum(q.sum(people_years, { as_object: false }))
    );
  },
});
