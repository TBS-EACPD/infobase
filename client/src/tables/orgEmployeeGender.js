      ...
      this.add_col({
        ...
        description: {
          en:
            ...
            people_years_short_second[ix],
          fr:
            "Correspond à personnel actif par sexe, au 31 mars " +
            people_years_short_second[ix],
        },
      });
    });
...
Statistics.create_and_register({
  id: "orgEmployeeGender_dept_info",
  table_deps: ["orgEmployeeGender"],
  level: "dept",
  compute: (subject, tables, infos, add, c) => {
    const table = tables.orgEmployeeGender;
    const q = table.q(subject);
    c.dept = subject;

    const all_years = q.get_top_x(["gender"].concat(people_years), Infinity, {
      zip: true,
    });
    ...
    const all_years_filtered = _.filter(
      all_years,
      (d) => d[0] !== gender.na.text && d[0] !== gender.sup.text
    );
    ...
    if (all_years_filtered.length >= 1) {
      ...
      const avg_percent_shares = _.map(
        all_years_filtered,
        (d) => d3.sum(_.tail(d)) / five_year_total
      );
      ...
      add(
        "head_count_gender_top_avg_percent_NA_included",
        _.max(avg_percent_shares)
      );
      add(
        "head_count_gender_bottom_avg_percent_NA_included",
        _.min(avg_percent_shares)
      );
    } else {
