...
Statistics.create_and_register({
  id: "orgEmployeeFol_dept_info",
  table_deps: ["orgEmployeeFol"],
  level: "dept",
  compute: (subject, tables, infos, add, c) => {
    const table = tables.orgEmployeeFol;
    const q = table.q(subject);
    c.dept = subject;

    const all_years = q.get_top_x(["fol"].concat(people_years), Infinity, {
      zip: true,
    });
    ...
    const all_years_filtered = _.filter(
      all_years,
      (d) => d[0] !== fol.na.text && d[0] !== fol.sup.text
    );
    ...
    if (all_years_filtered.length >= 1) {
      ...
      add(
        "head_count_fol_top_avg_percent_NA_included",
        _.max(avg_percent_shares)
      );
      add(
        "head_count_fol_bottom_avg_percent_NA_included",
