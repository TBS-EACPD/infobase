...
import React, { Fragment, JSXElementConstructor, useState } from "react";
...
import { _DisplayTable } from "./DisplayTable";
import { _DisplayTableState } from "./DisplayTable";
import { FilterOption } from "./DisplayTable";
...
export const DropdownFilter = ({
  ...
  column_searches,
}: {column_key: string,set_filter_options: any,filter_options_by_column: {[col_keys:string]: [FilterOption]},column_searches: {[keys:string]: string} }) => {
  const [search, set_search] = useState("");
  const clean_search_string = (string: string) =>
      ...
      dropdown_content={
        <div>
          <DebouncedTextInput
            ...
            defaultValue={search}
            updateCallback={(search_value: string) => set_search(search_value)}
            debounceTime={300}
          />
          {filtered_list.length > 0 ? (
            <>
              <div className="display-table-dropdown-filter">
                {_.map(filtered_list, ({ id, label, active, color }: {id: string, label: string, active: boolean, color: string}) => (
                  <CheckBox
                    id={id}
                    key={id}
                    color={color}
...
export const DisplayTableCopyCsv = ({ csv_string }: {csv_string: string}) => (
  <WriteToClipboard
...
export const DisplayTableColumnToggle = ({ columns }: {columns: JSX.Element}) => (
  <DropdownMenu
...
const download_csv = (csv_string: string, table_name: string | undefined) => {
  const uri =
...
export const DisplayTableDownloadCsv = ({ csv_string, table_name }: {csv_string: string, table_name: string | undefined}) => (
  <Fragment>
    <button
...
const _is_ellipsis = (page_options: number[], page_num: number, index: number) =>
  index !== 0 &&
...
export const SelectPage = ({
  ...
  change_page,
  num_col,
}: {current_page: number, num_pages: number, change_page: _DisplayTable["change_page"],num_col:number}) => {
  /*
  ...
  const extend_options_left = (option_range: any, page_count: number, current_page: number) => {
    console.log(typeof(option_range));
    if (_.first(option_range) === 1) {
      //option range sees page 1, add 2 to the end
      return [
        ...option_range,
       (_.last(option_range) as any) + 1,
       (_.last(option_range) as any) + 2,
      ];
    } else if (_.first(option_range) === 2) {
      //option range sees page 2 (page 1 is not visible so manually add it)
      return [1, ...option_range, (_.last(option_range) as any) + 1];
      ...
      return [
        1,
        (_.first(option_range) as any) - 2,
        (_.first(option_range) as any) - 1,
        ...option_range,
      ];
    } else if (current_page > page_count - 3) {
  ...
  const extend_options_right = (option_range: any, page_count: number, option_window: number) => {
    if (
      ...
      return [
        ...option_range,
        ..._.range((_.last(option_range) as any) + 1, page_count + 1),
      ];
    } else {
      return option_range;
    }
  };
  const get_page_options = (page_count: number, current_page: number, option_window = 2) => {
    ...
    const raw_page_options: List<number> = _.chain(
      ...
      .filter((page_index: number) => _.inRange(page_index, 1, page_count + 1))
      .uniq()
      .sortBy()
      .value();
    ...
    if (page_count > option_window * 2 + 5) {
      if (raw_page_options.length === option_window * 2 + 5) {
        ...
        if (!_.includes(raw_page_options, 1)) {
          ...
          return [1, (_.first(raw_page_options) as any) - 1, ...(raw_page_options as any)];
      ...
      <td colSpan={num_col}>
        <div style={{ padding: "10px" }}>
          <div className="row">
            ...
            >
              {_.map(page_options, (page_num, index: number) => (
                <button
                  key={page_num}
...
export const SelectPageSize = ({
  ...
  num_items,
  num_options_max,
}: {selected: number | undefined, on_select: _DisplayTable["change_page_size"], page_size_increment: number,num_items:number,num_options_max:number}) => {
  const num_options =
