...
import type { _DisplayTable, FilterOption } from "./DisplayTable";
...
export const DisplayTableColumnToggle = ({
  columns,
}: {
  columns: JSX.Element;
}) => (
...
export const DisplayTableDownloadCsv = ({
  csv_string,
  table_name,
}: {
  csv_string: string;
...
export const SelectPage = ({
  ...
  current_page: number;
  num_pages: number;
  change_page: _DisplayTable["change_page"];
  num_col: number;
}) => {
  /*
  ...
  const extend_options_left = (
    ...
    if (_.first(option_range) === 1) {
      ...
      return [
        ...option_range,
        (_.last(option_range) as number) + 1,
        (_.last(option_range) as number) + 2,
      ];
    } else if (_.first(option_range) === 2) {
      //option range sees page 2 (page 1 is not visible so manually add it)
      return [1, ...option_range, (_.last(option_range) as number) + 1];
      ...
      return [
        1,
        (_.first(option_range) as number) - 2,
        (_.first(option_range) as number) - 1,
        ...option_range,
      ];
    } else if (current_page > page_count - 3) {
      ...
      return [
        1,
        (_.first(option_range) as number) - 3,
        (_.first(option_range) as number) - 2,
        (_.first(option_range) as number) - 1,
        ...option_range,
      ];
  ...
  const extend_options_right = (
    ...
    if (
      ...
      return [
        ...option_range,
        ..._.range((_.last(option_range) as number) + 1, page_count + 1),
      ];
    } else {
      return option_range;
    }
...
export const SelectPageSize = ({
  ...
  on_select: _DisplayTable["change_page_size"];
  page_size_increment: number;
  num_items: number;
  num_options_max: number;
}) => {
  const num_options =
