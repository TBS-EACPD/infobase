...
export const AccessibleTablePicker = ({
  ...
  <Fragment>
    ...
    <select
      aria-label={text_maker("select_dataset_type")}
...
class TablePicker extends React.Component {
  constructor(props) {
    ...
    this.tables = _.chain(Table.store.get_all())
    ...
    this.linkage = _.chain(Table.store.get_all())
      .reject("reference_table")
      .map((table_obj) =>
          ...
          <div>
            <div className="row item-cloud-row">
              {_.map(items_split, (item_group, group_index) => (
                <div
                  key={group_index}
                  className="col-12 col-lg-4 item-cloud-col"
                >
                  <TransitionGroup>
