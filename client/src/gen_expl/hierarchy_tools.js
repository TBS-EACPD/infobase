...
function filter_hierarchy(flat_nodes, filter_func, { leaves_only=false, markSearchResults=false }){
  ...
  _.each(positive_search_results, node => {
    ...
    let done = false;
    while(!done){
      const parent_node = current.parent_id && old_nodes_by_id[current.parent_id];
...
function toggleExpandedFlat(flat_nodes, node, { toggleNode, expandAllChildren, collapseAllChildren }){
...
function simplify_hierarchy(flat_nodes, is_simple_parent, bottom_layer_filter){
  ...
  function get_simple_parent(node){
    ...
    while(suspected_simple_parent.parent_id){
      if(is_simple_parent(suspected_simple_parent, node) ){
        ...
        suspected_simple_parent = old_nodes_by_id[suspected_simple_parent.parent_id];
  ...
  const ancestors = _.chain(ancestors_by_id)
    .values()
    .uniqBy('id')
    .value();
