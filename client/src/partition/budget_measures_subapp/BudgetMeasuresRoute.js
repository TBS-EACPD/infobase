...
const footnotes_by_year = {
  ...
  2019: [
    <TextMaker
      key={1}
      text_key="budget2019_biv_includes_excludes_note"
      el="div"
    />,
  ],
};
...
const validate_route = (props) => {
  const {
    history,
    match: {
      params: { first_column, selected_value, budget_year },
    },
  } = props;

  const first_column_is_valid = _.includes(first_column_ids, first_column);
  const selected_value_is_valid = _.includes(
  ...
  if (
    first_column_is_valid &&
    selected_value_is_valid &&
    budget_year_is_valid
  ) {
    return true;
  } else {
...
export default class BudgetMeasuresRoute extends React.Component {
  ...
  conditionallyLoadOnMountAndUpdate() {
    ...
    const { loading, loaded_years } = this.state;

    if (
      loading &&
      !_.isUndefined(budget_year) &&
      !_.includes(loaded_years, budget_year)
    ) {
      ensure_loaded({
        ...
        this.setState({
          loading: false,
          loaded_years: [...loaded_years, budget_year],
        });
      });
    }
  }
  ...
  render() {
      ...
      <Fragment>
        ...
        {!loading && (
          <div className="budget-measures">
            <div className="budget-measures-top-text">
              <TextMaker
                ...
                args={{
                  ...
                  main_estimates_budget_link:
                    main_estimates_budget_links[year_value],
                }}
              />
              <Details
                ...
                content={
                  <TextMaker
                    ...
                    args={{
                      ...calculate_budget_stats(
                        get_year_value_from_budget_year(budget_year)
                      ),
                      budget_year: get_year_value_from_budget_year(budget_year),
                      budget_data_source_date:
                        budget_data_source_dates[year_value],
                    }}
                  />
                }
              />
            ...
            {window.is_a11y_mode && (
              <BudgetMeasuresA11yContent year_value={year_value} />
            )}
          </div>
        )}
      </Fragment>
    ...
    return (
      ...
      >
        <h1>{text_maker("budget_route_title")}</h1>
        ...
        {budget_years.length > 1 && (
          <Fragment>
            <TabbedControls
              ...
              tab_options={_.map(budget_year_options, (budget_year_option) => ({
                key: budget_year_option,
                label: `${text_maker(
                  "budget_name_header"
                )} ${get_year_value_from_budget_year(budget_year_option)}`,
