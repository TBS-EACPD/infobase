...
const standard_update = (diagram, props) => {
  const data = budget_measures_hierarchy_factory(props.year_value, props.selected_value, props.first_column, props.filtered_chapter_keys);
  const dont_fade = [];
...
const update_with_search = (diagram, props) => {
  ...
  const search_tree = budget_measures_hierarchy_factory(props.year_value, props.selected_value, props.first_column, props.filtered_chapter_keys);
...
const render_diagram = (diagram, props, data, data_wrapper_node_rules, dont_fade) => {
  const displayed_measure_count = _.filter(BudgetMeasure.get_all(), measure => {
    ...
    return !measure_is_filtered_out_for_year && !measure_is_filtered_out_for_chapter && !measure_is_filtered_out_for_value;
  }).length;
