...
const calculate_budget_stats = (year) => {
  ...
  const reduce_by_budget_value = (data) =>
    _.chain(budget_values)
      ...
      .fromPairs()
      .value();
  const rolled_up_data_rows = _.map(all_budget_measures, (budget_measure) =>
    reduce_by_budget_value(budget_measure.data)
  );

  const total_allocations_by_programs_and_internal_services = _.chain(
    all_budget_measures
  )
    .flatMap((budget_measure) =>
      _.map(budget_measure.data, (data_row) =>
        _.map(data_row.program_allocations, ({ subject_id, allocated }) => [
          subject_id,
          allocated,
        ])
      )
    ...
    .flatten()
    .groupBy((program_allocation_pairs) =>
      /ISS/.test(program_allocation_pairs[0]) ? "internal_services" : "programs"
    )
    .mapValues((grouped_program_allocation_pairs) =>
      _.chain(grouped_program_allocation_pairs)
        .groupBy((program_allocation_pair) => program_allocation_pair[0])
        .mapValues((program_allocation_pairs) =>
          _.reduce(
  ...
  const vote_count =
    year === "2018" ? 1 : _.flatMap(all_budget_measures, "data").length;
  const multiple_votes = vote_count > 1;

  const allocated_to_measure_count = _.filter(
    rolled_up_data_rows,
    (data_row) => data_row.allocated
  ).length;

  const allocated_to_program_count = _.size(
    total_allocations_by_programs_and_internal_services.programs
  );

  const allocated_to_internal_services_count = _.size(
    total_allocations_by_programs_and_internal_services.internal_services
  );

  const allocated_to_all_program_count =
    allocated_to_program_count + allocated_to_internal_services_count;

  const total_internal_service_allocated = _.reduce(
    total_allocations_by_programs_and_internal_services.internal_services,
    (sum, internal_service_total_allocation) =>
      sum + internal_service_total_allocation,
    0
  );
  ...
  const fully_withheld_funds_count = _.filter(
    rolled_up_data_rows,
    (data_row) =>
      data_row.funding !== 0 && data_row.funding === data_row.withheld
  ).length;

  const totally_funded_count =
  ...
  return {
    ...
    total_allocated_share:
      funding_data_totals.allocated / funding_data_totals.funding,
    total_withheld: funding_data_totals.withheld,
    total_withheld_share:
      funding_data_totals.withheld / funding_data_totals.funding,
    total_remaining: funding_data_totals.remaining,
    total_remaining_share:
      funding_data_totals.remaining / funding_data_totals.funding,
