...
const post_traversal_search_string_set = (node) => {
  ...
  if ( node.data.type === "dept" && node.data.id !== 9999 ){
    node.data.search_string += _.deburr(
      `${node.data.dept_code} ${node.data.abbrev} ${node.data.applied_title}`.toLowerCase()
    );
  }
};
  ...
  if ( _.isEmpty(program_allocations) ){
    ...
    const program_allocation_nodes = _.map(program_allocations, ({subject_id, allocated}) => {
      ...
      if ( !_.isUndefined(program_or_crso) ){
        return {
          ...program_or_crso,
          ...{description: year_value === "2018" ? program_or_crso.description : ""},
          ...type_and_value,
        };
      } else {
        ...
        const budgetMeasureNodes = _.chain( BudgetMeasure.get_all() )
          ...
          .map( budgetMeasure => {
            ...
            return {
              ...
              description: has_no_description ? year_value === "2018" && text_maker("not_available") : description,
              notes: year_value === "2018" && !has_no_description && text_maker("budget2018_measure_description_values_clarification"),
          ...
          .map(data_row => {
            if (data_row.org_id === "non_allocated"){
              ...
              return {
                ...dept,
                type: "dept",
                description: year_value === "2018" ? dept.mandate : data_row.description,
                value: data_row[selected_value],
                parent_measure_id: node.measure_id,
              };
            }
      ...
      if (node.id === "root"){
        ...
        const budgetMeasureNodes = _.map(node.data_rows, data_row => {
          ...
          return {
            ...
            type: "budget_measure",
            chapter_key: budgetMeasure.chapter_key,
            description: has_no_description ? text_maker("not_available") : description,
        ...
        const measure_funding_nodes = _.chain( BudgetMeasure.get_all() )
          ...
          .map( budgetMeasure => {
            ...
            return {
              ...
              type: /net_adjust/.test(budgetMeasure.id) ? "net_adjust" : "budget_measure",
              value_type: "funding",
              description: has_no_description ? year_value === "2018" && text_maker("not_available") : budgetMeasure.description,
              notes: !has_no_description && year_value === "2018" ? text_maker("budget2018_measure_description_values_clarification") : false,
