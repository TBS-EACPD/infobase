...
const roll_up_children_values = (node) => {
  return _.reduce(
    node.children,
    (sum, child_node) => sum + child_node.value,
    0
  );
};
...
const get_node_submeasures = (node, year_value, selected_value) => {
  ...
  if (node.data.type === "program_allocation") {
    ...
    org_id =
      node.parent.data.type === "dept"
        ? node.parent.data.id
        : node.parent.parent.data.id;
    measure_id =
      node.parent.data.type === "budget_measure"
  ...
  const node_submeasures = _.chain(
    BudgetMeasure.lookup_measure(year_value, measure_id).data
  )
    .flatMap(({ submeasure_breakouts }) => submeasure_breakouts)
    .filter(
      (submeasure_breakout) =>
        org_id !== "net_adjust" &&
        _.toString(org_id) === submeasure_breakout.org_id
    )
    .map((submeasure_breakout) => ({
      ...submeasure_breakout,
      value: program_or_crso_id
        ? _.chain(submeasure_breakout.program_allocations)
            .find(({ subject_id }) => subject_id === program_or_crso_id)
            .thru((program_allocation) =>
              _.get(program_allocation, selected_value)
            )
            .value()
        : submeasure_breakout[selected_value],
    }))
    .filter(
      (submeasure) => !_.isUndefined(submeasure.value) && submeasure.value !== 0
    )
  ...
  if (node.value_type === "funding") {
    ...
    node.children = _.filter(node.children, (child_node) => {
      return _.isUndefined(child_node.children)
        ...
        : _.some(
            child_node.children,
            (child_node_child) => child_node_child.value !== 0
          );
    });
  }
};
...
const make_program_allocation_nodes = (year_value, measure_id, org_id) => {
  const program_allocations = _.chain(
    BudgetMeasure.lookup_measure(year_value, measure_id).data
  )
    .filter(
      (data_row) =>
        +data_row.org_id === org_id && !_.isEmpty(data_row.program_allocations)
    )
  ...
  if (_.isEmpty(program_allocations)) {
    ...
    const program_allocation_nodes = _.map(
      program_allocations,
      ({ subject_id, allocated }) => {
        ...
        if (!_.isUndefined(program_or_crso)) {
          return {
            ...
            ...{
              description:
                year_value === "2018" ? program_or_crso.description : "",
            },
            ...type_and_value,
          };
        } else {
          ...
          window.is_dev &&
            console.warn(`Budget hierarchy: missing program ${subject_id}`); // eslint-disable-line
...
const budget_measure_first_hierarchy_factory = (year_value, selected_value) => {
  return d3
      ...
      (node) => {
        if (node.id === "root") {
          const budgetMeasureNodes = _.chain(BudgetMeasure.get_all())
            .filter((budgetMeasure) => budgetMeasure.year === year_value)
            .map((budgetMeasure) => {
              const description = budgetMeasure.description;

              const has_no_description = _.isEmpty(description);

              return {
                ...budgetMeasure,
                type: /net_adjust/.test(budgetMeasure.id)
                  ? "net_adjust"
                  : "budget_measure",
                description: has_no_description
                  ? year_value === "2018" && text_maker("not_available")
                  : description,
                notes:
                  year_value === "2018" &&
                  !has_no_description &&
                ...
                value: _.reduce(
                  budgetMeasure.data,
                  (sum, data_row) => sum + data_row[selected_value],
                  0
                ),
              };
            })
            ...
            .map((data_row) => {
              if (data_row.org_id === "non_allocated") {
                ...
                return {
                  ...
                  type: "dept",
                  description:
                    year_value === "2018" ? dept.mandate : data_row.description,
                  value: data_row[selected_value],
                  parent_measure_id: node.measure_id,
                };
              }
    ...
    .eachAfter((node) =>
      post_traversal_modifications(node, year_value, selected_value)
    )
    .sort(absolute_value_sort_net_adjust_biased);
};

const dept_first_hierarchy_factory = (year_value, selected_value) => {
  const filtered_budget_measure_funds_by_org_id = _.chain(
  ...
  return d3
    .hierarchy(
      ...
      (node) => {
        if (node.id === "root") {
          const deptNodes = _.map(
            filtered_budget_measure_funds_by_org_id,
            (data_rows, org_id) => {
              if (org_id === "non_allocated") {
                ...
                const net_adjust_measure = BudgetMeasure.lookup_measure(
                  year_value,
                  `net_adjust`
                );
                ...
                return {
                  ...dept,
                  type: "dept",
                  description: year_value === "2018" && dept.mandate,
                  data_rows,
                };
              }
            }
          ...
          const budgetMeasureNodes = _.map(node.data_rows, (data_row) => {
            const budgetMeasure = BudgetMeasure.lookup_measure(
              year_value,
              data_row.measure_id
            );

            const description =
              budgetMeasure.description || data_row.description;
            ...
            return {
              ...
              description: has_no_description
                ...
                : description,
              notes:
                !has_no_description && year_value === "2018"
                  ? text_maker(
                      "budget2018_measure_description_values_clarification"
                    )
                  : false,
          ...
          selected_value === "allocated" &&
          node.type === "budget_measure"
        ) {
...
const budget_overview_hierarchy_factory = (year_value) => {
  return d3
          ...
          const measure_funding_nodes = _.chain(BudgetMeasure.get_all())
            ...
            .map((budgetMeasure) => {
              ...
              return {
                ...
                value: _.reduce(
                  budgetMeasure.data,
                  (sum, data_row) => sum + data_row.funding,
                  0
                ),
                chapter_key: budgetMeasure.chapter_key,
              };
    ...
    .eachAfter((node) => {
      ...
      if (
        node.data.type === "dept" ||
        node.data.type === "program_allocation"
      ) {
...
export function budget_measures_hierarchy_factory(
  year_value,
  selected_value,
  first_column
) {
