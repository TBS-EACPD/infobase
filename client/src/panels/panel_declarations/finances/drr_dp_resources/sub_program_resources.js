    ...
    resources: {
      ftes:
        (doc === latest_drr_doc_key
          ? sub.fte_pa_last_year
          : sub.fte_planning_year_1) || 0,
      spending:
        (doc === latest_drr_doc_key
          ? sub.spend_pa_last_year
          : sub.spend_planning_year_1) || 0,
    },
...
function create_resource_hierarchy({ program, doc }) {
  ...
  const d3_hierarchy = d3.hierarchy(root, (node) => {
    if (node.root) {
      return _.map(SubProgramEntity.sub_programs(program.id), (sub) =>
        sub_to_node(sub, doc)
      );
    } else {
      const { id } = node;
      ...
      return _.map(SubProgramEntity.sub_programs(id), (sub) =>
        sub_to_node(sub, doc)
      );
    }
  });
...
const get_non_col_renderer = ({ doc }) => ({ node }) => {
  const {
    data: { description, notes, resource_table_props },
  } = node;
    ...
    <div>
      <div
        style={{
          ...
          borderTop: `1px solid ${window.infobase_color_constants.separatorColor}`,
        }}
      >
        {description}
      </div>
        ...
        >
          ...
          <ul>
            {_.map(notes, (note) => (
              <li key={note}>
                <div
                  dangerouslySetInnerHTML={sanitized_dangerous_inner_html(note)}
                />
              </li>
            ))}
          </ul>
...
const get_col_defs = ({ doc }) => [
  ...
  },
  {
    ...
    header_display: (
      <TM
        k={
          /dp/.test(doc) ? "planned_spending_header" : "actual_spending_header"
        }
        args={{
...
const sub_program_resource_scheme = {
  ...
  get_sort_func_selector: () =>
    provide_sort_func_selector("sub_program_resource"),
  get_props_selector: () => {
...
class SubProgramResourceTree extends React.Component {
  ...
  render() {
    ...
    if (!has_dp_data || !has_drr_data) {
      //don't wrap the inner content in a tab layout
...
const SubProgramResourceTreeContainer = ({
  ...
  const initialState = {
    ...
    [scheme_key]: {
      ...initial_sub_program_state,
      ...initial_scheme_state_slice,
    },
  };
...
export const declare_sub_program_resources_panel = () =>
  declare_panel({
    ...
    panel_config_func: (level, panel_key) => ({
      ...
      calculate(subject) {
        ...
        const drr_sub_subs = _.chain(drr_subs)
          .map(({ id }) => SubProgramEntity.sub_programs(id))
          .flatten()
          .filter("has_drr_resources")
          .value();

        const dp_subs = _.filter(subs, "has_dp_resources");
        const dp_sub_subs = _.chain(drr_subs)
          .map(({ id }) => SubProgramEntity.sub_programs(id))
          .flatten()
          .filter("has_dp_resources")
          .value();
        ...
        const has_drr_data =
          counts &&
          counts[`${latest_drr_doc_key}_total`] > 0 &&
          _.nonEmpty(drr_subs);
