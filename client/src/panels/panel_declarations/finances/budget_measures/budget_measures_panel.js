...
const { BudgetMeasure, Dept, Program, CRSO } = Subject;
...
const crso_program_calculate = (subject, info, options, years_with_data) => {
  ...
  const get_program_measures_with_data_filtered = (year) =>
...
const calculate_functions = {
  gov: function (subject, info, options, years_with_data) {
    const get_all_measures_with_data_rolled_up = (year) =>
      _.chain(BudgetMeasure.get_all())
        ...
        .map((measure) => ({
          ...
          measure_data: _.chain(budget_values)
            ...
            .map((key) => [
              ...
              _.reduce(
                measure.data,
                (total, data_row) => total + data_row[key],
                0
              ),
            ])
            .fromPairs()
  ...
  dept: function (subject, info, options, years_with_data) {
    ...
    const get_org_measures_with_data_filtered = (year) =>
      _.chain(BudgetMeasure.get_all())
        .filter(
          (measure) =>
...
class BudgetMeasurePanel extends React.Component {
  ...
  mountAndUpdate() {
    ...
    const { loading, selected_year } = this.state;
  ...
  render() {
    ...
    const { years_with_data, selected_year, loading } = this.state;
    ...
    const above_tab_text = (
      <div className="frow">
        {years_with_data.length === 1 &&
          (subject.level === "dept" || treatAsProgram(subject)) && (
            <div className="fcol-md-12 fcol-xs-12 medium_panel_text text">
              {(panel_args.subject.level === "dept" ||
    ...
    return (
      <Fragment>
        ...
        <div className="tabbed-content">
          <TabbedControls
            tab_callback={(year) =>
              this.setState({ loading: true, selected_year: year })
            }
            tab_options={_.map(
...
class BudgetMeasureHBars extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    const {
      panel_args: { get_data, get_info, subject },
      selected_year,
    } = props;

    const { selected_grouping, selected_value } = state;
    ...
    const value_options =
      treatAsProgram(subject) || valid_selected_grouping === "programs"
        ? [
            {
              id: "allocated",
    ...
    const valid_selected_value =
      _.filter(
        value_options,
        (value_option) => value_option.id === selected_value
      ).length === 1
        ? selected_value
        : value_options[0].id;
    ...
    const get_program_allocation_data_from_dept_data = (data) => {
      return _.chain(data)
        ...
        .map((program_allocation, program_id) => {
          ...
          if (!_.isUndefined(program)) {
            ...
            window.is_dev &&
              console.warn(`Budget panel: missing program ${program_id}`); // eslint-disable-line
    ...
    const get_top_data = (data, grouping, value) => {
      ...
      const others = {
        ...
        chapter_key: null,
        description: "",
        name: text_maker("x_smaller_" + grouping, {
          smaller_items_count: sorted_data.length - TOP_TO_SHOW,
        }),
        section_id: null,
        year: selected_year,
      };
    ...
    const filtered_data_by_selected_group = _.filter(
      {
        measures: get_top_data(
          data,
          valid_selected_grouping,
          valid_selected_value
        ),
        orgs: get_top_data(
      ...
      (row) =>
        valid_selected_value !== "funding_overview"
  ...
  render() {
    ...
    const tick_map = _.chain(data)
      .map((row) => [
        ...
        row.__typename === "BudgetMeasure"
          ? row.chapter_key &&
            BudgetMeasure.make_budget_link(row.chapter_key, row.ref_id)
          : row.href,
      ])
      .fromPairs()
      .value();
    ...
    const text_area = (
      <div className="frow">
        <div className="fcol-md-12 fcol-xs-12 medium_panel_text text">
          {subject.level === "gov" && (
            <TM k={"gov_budget_measures_panel_text"} args={panel_text_args} />
          )}
          {subject.level === "dept" && (
            <TM k={"dept_budget_measures_panel_text"} args={panel_text_args} />
          )}
          {treatAsProgram(subject) && (
            <TM
    ...
    if (window.is_a11y_mode) {
      ...
      const biv_value_colors = d3
        .scaleOrdinal()
        .range(_.at(newIBLightCategoryColors, [0, 2, 4]));

      const nivo_default_props = {
        data: _.map(data, (measure) =>
          _.chain(measure)
      ...
      const nivo_mobile_props = _.merge({}, nivo_default_props, {
        margin: {
          right: 10,
          left: 250,
          top: 80,
          ...
          <div className="row" style={{ marginBottom: "10px" }}>
            <div className="col-sm-6" align="center">
              ...
              <Select
                ...
                options={_.map(grouping_options, ({ name, id }) => ({
                  id,
                  display: name,
                }))}
            ...
            <div className="col-sm-6" align="center">
              ...
              <Select
                ...
                options={_.map(value_options, ({ name, id }) => ({
                  id,
                  display: name,
                }))}
...
export const declare_budget_measures_panel = () =>
  declare_panel({
    ...
    panel_config_func: (level_name, panel_key) => ({
      ...
      calculate: (subject, info, options) => {
        const years_with_data =
          level_name === "gov"
            ? budget_years
            : _.filter(budget_years, (year) =>
                subject.has_data(`budget${year}_data`)
              );

        return (
          !_.isEmpty(years_with_data) &&
          calculate_functions[level_name](
            subject,
            info,
            options,
            years_with_data
