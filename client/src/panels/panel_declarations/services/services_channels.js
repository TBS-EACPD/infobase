...
class ServicesChannelsPanel extends React.Component {
  ...
  render() {
    ...
    const all_services = _.chain(panel_args.services)
      ...
      .flatMap(_.toPairs)
      .groupBy(_.first)
      .mapValues((key_value_pairs) =>
        _.reduce(key_value_pairs, (sum, [key, value]) => sum + value, 0)
      )
    ...
    const selected_service =
      selected_service_id === "all"
        ...
        : _.find(
            panel_args.services,
            (serv) => serv.id === selected_service_id
          ) || _.first(panel_args.services);

    const selected_title = `${text_maker("channels_enquiry_vectors")} ${
    ...
    const feedback_channel_statuses = _.map(
      all_feedback_channels,
      (channel) => ({
        id: channel,
        label: channel,
        color: _.includes(selected_service.feedback_channels, channel)
          ? colors(0)
          : null,
      })
    );
          ...
          <Fragment>
            ...
            <div className="frow">
              <div className="fcol-md-9" aria-hidden={true}>
                <div className="medium_panel_text">{selected_title}</div>
                <NivoResponsiveBar
                  data={bar_data}
                  indexBy="label"
              ...
              {selected_service_id !== "all" && (
                <div className="fcol-md-3" aria-hidden={true}>
                  ...
                  <StandardLegend items={feedback_channel_statuses} />
                </div>
              )}
            </div>
          </Fragment>
