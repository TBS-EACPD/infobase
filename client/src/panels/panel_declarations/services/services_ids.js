...
const ServicesIdPanel = ({ panel_args }) => {
  ...
  const bar_data = _.chain(data_keys)
    .map((key) => ({
      uses_this_id: _.reduce(
        panel_args.services,
        (sum, serv) => (serv[key] ? sum + 1 : sum),
        0
      ),
      doesnt_use_id:
        _.size(panel_args.services) -
        _.reduce(
          panel_args.services,
          (sum, serv) => (serv[key] ? sum + 1 : sum),
          0
    ...
    .each((obj) =>
      _.each(obj, (value, key) => {
        if (_.includes(label_keys, key))
          obj[text_maker(`label_${key}`)] = obj[key];
      })
    )
    .value();
...
export const declare_services_ids_panel = () =>
  declare_panel({
      ...
      render({ calculations, sources }) {
        ...
        return (
          <InfographicPanel
            ...
            sources={sources}
          >
            <ServicesIdPanel panel_args={panel_args} />
          </InfographicPanel>
        );
      },
    }),
