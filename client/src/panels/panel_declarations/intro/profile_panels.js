  ...
  panel_config_func: (level, panel_key) => ({
    calculate: (subject) => {
      switch(level){
        ...
        case 'tag': {
          ...
          if ( subject.root.id === "HI" && !_.isUndefined(subject.lookups) && !_.isEmpty(subject.lookups) ){
    ...
    render({calculations}){
      ...
      const labels_and_items = _.chain(profile_fields)
        .map( ([label_key, item]) => {
          const label = label_key !== "legislation" ? // legislation text contains markup (to fix a confusing line break), needs to be rendered through innerHTML
            text_maker(label_key) : 
            <div dangerouslySetInnerHTML={{__html: text_maker(label_key)}} />;
