...
const EmployeeAgePanel = ({
  ...
  const calculations = useMemo(() => {
    ...
    const age_group = data.age_group
      ...
      .map((row) => {
        ...
        else if (row.dimension.includes("30-34") || 
        ...
        else if (row.dimension.includes("40-44") || 
        ...
        else if (row.dimension.includes("50-54") || 
        ...
        else if (row.dimension.includes("60-64") || 
        ...
        else if (row.dimension.match(/^(Suppressed Data|Données supprimées)$/i)) {
    ...
    age_group.forEach(group => {
      if (consolidatedGroups[group.label]) {
        ...
        if (group.five_year_percent !== null && !isNaN(group.five_year_percent)) {
    ...
    const finalAgeGroups = Object.values(consolidatedGroups)
      .map(group => ({
        label: group.label,
        data: group.data,
        five_year_percent: group.count > 0 ? group.five_year_percent / group.count : 0,
        active: true
      }))
      .filter(group => _.some(group.data, val => val !== null && val !== 0))
      .sort((a, b) => {
    ...
    if (subject_type === "dept" && govData && govData.average_age && govData.average_age.length > 0) {
      avg_age.push({
  ...
  if (!calculations || !calculations.age_group || calculations.age_group.length === 0) {
    return null;
  }
