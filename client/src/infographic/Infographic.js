      ...
      _.chain(panel_keys)
        .map((panel_key) => {
          ...
          const active_panel_filter_keys = _.chain(panel_filter)
            ...
            .value();
          const show_panel =
            _.intersection(active_panel_filter_keys, panel_obj.depends_on)
              .length > 0 || show_all_panels_bubble_type;
          return (
            show_panel && (
              <PanelRenderer
                panel_key={panel_key}
            ...
            <Details
              summary_content={
                <div>
                  ...
                  <TM
                    ...
                    k="panels_status"
                    args={{
                      number_of_active_panels: panel_renderers.length,
              ...
              content={
                <fieldset>
                  ...
                  <MultiColumnList
                    list_items={_.map(panel_filter, (checked, dependency) => (
                      <CheckBox
                        id={dependency}
      ...
      if (
        ...
        const panel_filter = _.chain(panel_keys)
          .map((panel_key) =>
            _.intersection(
              subject.table_ids,
              tables_for_panel(panel_key, subject.level)
            )
          )
          .flatten()
          .uniq()
        ...
        const total_number_of_panels = _.reduce(
          panel_keys,
          (count, panel_key) => {
            const panel_obj = PanelRegistry.lookup(panel_key, subject.level);
            const active_panel_filter_keys = _.chain(panel_filter)
