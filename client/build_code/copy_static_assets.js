...
function build_proj(PROJ) {
  ...
  const parsed_bilingual_models = _.mapValues(
    bilingual_model_files,
    (file_name) =>
      d3_dsv.csvParse(_.trim(file_to_str(public_dir_prefixer(file_name))))
  );
  ...
  _.each(["en", "fr"], (lang) => {
    ...
    fs.writeFileSync(`${footnotes_dir}/fn_${lang}_all.json.js`, all_footnotes);
  ...
  PROJ.well_known.forEach((f_name) =>
    copy_file_to_target_dir(f_name, well_known_dir)
  );
