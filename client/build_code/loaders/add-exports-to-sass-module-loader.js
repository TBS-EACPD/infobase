...
/* 
  ...
  accounting for right now. Reasonable for the convention within sass modules to be avoiding dynamic top level variables,
...
const test_variable_type_cases = (scss_module_path, variable_names) => {
  ...
  if (!_.isEmpty(non_camel_case_names)) {
    throw new Error(
      `${scss_module_path}: for consistency and to ensure interopability between SCSS modules and JS, top level SCSS variables (and preferably all SCSS variables) must be in camelCase. Check the following: ${_.join(
